...
import { Lock, MoreVertical, ArrowLeft, Phone, Video } from 'lucide-react';
...
export default function ChatView({ chat, currentUser, onSendMessage, onBack }: ChatViewProps) {
  ...
  const getChatHistory = () => {
    return chat.messages
      .map(
        (m) =>
          `${m.senderId === currentUser.id ? 'You' : participant?.name}: ${m.text}`
      )
      .join('\n');
  };
      ...
      <header className="flex items-center justify-between p-3 border-b bg-card shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {participant.status === 'online' && <div className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full bg-green-500 border-2 border-card" />}
          </div>
          <div>
